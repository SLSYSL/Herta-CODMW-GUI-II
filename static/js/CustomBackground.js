...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        if (savedCarouselIndex !== null) {
            currentCarouselIndex = parseInt(savedCarouselIndex);
        }
    ...
    function setDarkBackground() {
        const isDarkMode = document.querySelector('.dark-mode');
        if (isDarkMode) {
            document.querySelector('body').style.background = 'rgba(45, 45, 61)';
        } else {
    ...
    // 事件监听直接调用函数
    document.getElementById('themeToggle').addEventListener('click', setDarkBackground);
    document.getElementById('removeBackground').addEventListener('click', setDarkBackground);
        ...
        carouselImages.forEach((image, index) => {
            ...
            imgElement.addEventListener('click', function () {
                ...
                if (backgroundContainer) {
                    ...
                    // 保存当前点击的图片索引到localStorage
                    const clickedIndex = carouselImages.indexOf(image);
                    if (clickedIndex !== -1) {
                        currentCarouselIndex = clickedIndex;
                        try {
                            localStorage.setItem('currentCarouselIndex', currentCarouselIndex.toString());
                        } catch (e) {
                            console.error('保存当前轮播索引时出错:', e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.error('保存轮播图片为自定义背景时出错:', e);
                    }
                }
            });
        ...
        carouselIntervalId = setInterval(() => {
            if (backgroundContainer && carouselImages.length > 0) {
                ...
                setTimeout(() => {
                    ...
                    backgroundContainer.style.backgroundImage = `url("${currentImage}")`;
                    updateBackgroundFilter();

                    // 保存当前轮播索引到localStorage
                    try {
                        localStorage.setItem('currentCarouselIndex', currentCarouselIndex.toString());
                    } catch (e) {
                        console.error('保存当前轮播索引时出错:', e);
                    ...
                    // 将当前轮播图片保存为自定义背景
                    try {
                        localStorage.setItem('customBackground', currentImage);
                        // 清除已移除背景的标志
                        localStorage.removeItem('hasRemovedBackground');
                    } catch (e) {
                        console.error('保存轮播图片为自定义背景时出错:', e);
